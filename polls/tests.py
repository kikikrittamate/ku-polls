...
class QuestionModelTests(TestCase):
    ...
    def test_can_vote_with_ongoing_question(self):
        """
        can_vote returns True for questions whose pub_date is older than current time
        and end_date is newer than current time.
        """
        pub_date = timezone.now() - datetime.timedelta(days=1)
        end_date = timezone.now() + datetime.timedelta(days=7)
